...
{
public:
	iMouseDragOperationsHandler(ssRectangleSpace & r_ref, am_Uuid_T<opMouseDragTransform *> & o_ref) : rectangleSpace_ref_(r_ref), operations_ref_(o_ref) {};
	opMouseDragTransform * find_operation_from_screen_coordinates(int, int);
private:
	ssRectangleSpace & rectangleSpace_ref_;
	am_Uuid_T<opMouseDragTransform *> & operations_ref_;
};